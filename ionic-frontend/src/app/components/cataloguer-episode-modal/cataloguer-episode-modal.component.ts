...
import { EpisodePayload, EpisodeMarker, TrackData } from '../../models/cataloguer';
...
export class CataloguerEpisodeModalComponent implements OnInit {
    ...
    constructor() {
        addIcons({
            imageOutline, documentTextOutline, videocamOutline, informationCircleOutline,
            addCircleOutline, trashOutline, checkmarkCircle, musicalNotesOutline,
            textOutline, volumeHighOutline, timeOutline,
        });
    }
    ...
    private fetchExistingMarkers() {
        this.cataloguerService.getEpisodeMarkers(this.data.EpisodeID).subscribe({
            next: (times) => {
    ...
    private initForm() {
        ...
        this.episodeForm = this.fb.group({
            ...
            episodeNumber: [{ value: this.data?.EpisodeNumber || this.autoEpisodeNumber, disabled: true }],
            ...
            buttons: [
                { text: 'Annulla', role: 'cancel' },
                {
                    ...
                    handler: async () => {
                        ...
                        try {
                            // 🚀 Chiamata al Service
                            await firstValueFrom(this.cataloguerService.deleteEpisodeTrack(this.data.EpisodeID, type, lang));
    ...
    async save() {
        ...
        try {
            ...
            if (this.thumbnailFile()) {
                const res = await firstValueFrom(this.cataloguerService.uploadEpisodeMedia(this.thumbnailFile()!, 'thumbnail'));
                finalThumbURI = res.uri;
            }

            let finalRawVideoURI = null;
            if (this.videoFile()) {
                const res = await firstValueFrom(this.cataloguerService.uploadEpisodeMedia(this.videoFile()!, 'raw_video'));
            ...
            for (const track of this.audioTracksList()) {
                if (track.file) {
                    const res = await firstValueFrom(this.cataloguerService.uploadEpisodeMedia(track.file, 'track'));
