...
export class CataloguerEpisodeModalComponent implements OnInit {
    ...
    markersList = signal<
        { StartTime: number | null; EndTime: number | null; Type: string }[]
    >([])
    ...
    private fetchExistingMarkers() {
        ...
        this.http
            .get<
                any[]
            >(`http://localhost:3000/api/cataloguer/episodes/${this.data.EpisodeID}/times`, { headers })
            .subscribe({
                next: (times) => {
    ...
    async save() {
        ...
        try {
            ...
            if (this.thumbnailFile())
                finalThumbURI = await this.uploadThumbnail(
                    this.thumbnailFile()!
                )
