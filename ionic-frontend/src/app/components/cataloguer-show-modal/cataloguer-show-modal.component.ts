...
export class CataloguerShowModalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.isEditMode) {
      ...
      if (this.data.BannerURI) this.bannerPreview.set(`http://localhost:3000/${this.data.BannerURI}`);
    }
  ...
  private async uploadImage(file: File, type: 'thumbnail' | 'banner'): Promise<string> {
  ...
  async save() {
    ...
    try {
      ...
      if (this.thumbnailFile()) {
        finalThumbURI = await this.uploadImage(this.thumbnailFile()!, 'thumbnail');
      }
      if (this.bannerFile()) {
        finalBannerURI = await this.uploadImage(this.bannerFile()!, 'banner');
      }
