...
export const CataloguerGuard: CanActivateFn = (route, state) => {
  const router = inject(Router);
  const token = localStorage.getItem('token');

  if (token) {
    try {
      // Decodifichiamo il payload del JWT al volo per leggere i dati reali
      const decodedToken = jwtDecodeHelper(token); 
      
      // Permettiamo l'accesso se l'utente è un Catalogatore
  ...
  // Se i flag non sono validi o il token è corrotto, reindirizza alla pagina 403
