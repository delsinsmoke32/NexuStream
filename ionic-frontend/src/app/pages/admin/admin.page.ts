...
export class AdminPage implements OnInit {
  ...
  private baseUrl = 'http://localhost:3000/api/admin/users';
  ...
  changeRole(userId: number, event: any) {
    ...
    this.http.patch(`${this.baseUrl}/${userId}/roles`, bodyPayload, { headers: this.getAuthHeaders() }).subscribe({
      next: () => {
        this.presentToast("Ruoli aggiornati correttamente!", "success");
        
        // Aggiornamento reattivo del Signal locale: modifica solo l'utente mirato mantenendo intatto lo scroll
        this.users.update(currentUsers => 
          currentUsers.map(user => 
            user.UserID === userId 
              ? { ...user, isMod: bodyPayload.isMod, isCataloguer: bodyPayload.isCataloguer }
              : user
          )
        );
      },
      error: (err) => {
        ...
        this.presentToast("Errore durante l'aggiornamento dei privilegi.", "danger");
      }
    });
  }
