...
export class CataloguerPage implements OnInit {
    ...
    private updateSignalsAfterAdd(currentLvl: string, res: any, rawData: any) {
        ...
        const newDesc = JSON.stringify({
            it: rawData.description_it,
            en: rawData.description_en,
        })
    ...
    private buildEditExtraFields(
        ...
        if (currentLvl === 'shows') {
            if (payload.dateEnded !== undefined)
                extraFields.dateEnded = payload.dateEnded
            if (payload.thumbnailURI)
                extraFields.thumbnailURI = payload.thumbnailURI
            ...
            extraFields.SubLanguages =
                typeof item.SubLanguages === 'string'
                    ...
                    : item.SubLanguages || []
            if (payload.duration)
                extraFields.duration = parseInt(payload.duration, 10)
            if (payload.episodeNumber)
                extraFields.episodeNumber = parseInt(payload.episodeNumber, 10)
            if (payload.audioTracks?.length > 0)
                extraFields.audioTracks = payload.audioTracks
            if (payload.subTracks?.length > 0)
                extraFields.subTracks = payload.subTracks
    ...
    private getEditTranslations(item: any, payload: any) {
        return {
            it: {
                title:
                    payload.title_it?.trim() ||
                    this.getLangText(item.Title, 'it'),
                desc:
                    payload.description_it?.trim() ||
    ...
    private async sendEditPatches(
        currentLvl: 'shows' | 'seasons' | 'episodes',
        id: number,
        payload: any,
        extraFields: any,
        trans: any
        ...
        if (
            ...
            const payloadIt = {
                title_it: trans.it.title,
                description_it: trans.it.desc,
                title: trans.it.title,
                description: trans.it.desc,
                lang: 'it',
                ...extraFields,
            }
    ...
    private updateSignalsAfterEdit(
        currentLvl: string,
        id: number,
        item: any,
        payload: any,
        ...
        if (currentLvl === 'shows') {
            ...
            let updatedSubs = item.SubLanguages
                ...
                : []
            payload.subTracks?.forEach((t: any) => {
