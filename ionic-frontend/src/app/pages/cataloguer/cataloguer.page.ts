...
export class CataloguerPage implements OnInit {
    ...
    async openEditModalData(item: any) {
        ...
        try {
            ...
            if (currentLvl === 'shows') {
                ...
                if (item.Duration)
                    extraFields.duration = parseInt(item.Duration, 10)
                if (item.EpisodeNumber)
                    extraFields.episodeNumber = parseInt(item.EpisodeNumber, 10)
            }
    ...
    async managePropics() {
        ...
        const action = confirm(
            'Clicca OK per AGGIUNGERE la propic, ANNULLA per RIMUOVERLA.'
        )
        if (action) {
            this.http
                .post(
                ...
                .subscribe({
                    next: () =>
                        this.presentToast('Propic aggiunta!', 'success'),
                    error: (err) =>
                        this.presentToast(
                            err.error?.error || 'Errore',
                            'danger'
                        ),
                })
        } else {
