...
export class CataloguerService {
    ...
    getShows(page: number, limit: number, search: string = ''): Observable<CataloguerShow[]> {
    ...
    deleteShow(showId: number): Observable<any> {
        return this.http.delete(`${this.baseUrl}/shows/${showId}`, { headers: this.getAuthHeaders() });
    }
    ...
    deleteSeason(seasonId: number): Observable<any> {
        return this.http.delete(`${this.baseUrl}/seasons/${seasonId}`, { headers: this.getAuthHeaders() });
    }
    ...
    addItem(level: 'shows' | 'seasons' | 'episodes', payload: any): Observable<any> {
    ...
    uploadShowImage(file: File, type: 'thumbnail' | 'banner'): Observable<{ uri: string }> {
        const formData = new FormData();
        formData.append(type, file, file.name); 

        const endpoint = type === 'thumbnail' ? 'api/upload/show_thumbnails' : 'api/upload/banners';
        return this.http.post<{ uri: string }>(endpoint, formData, { headers: this.getAuthHeaders() });
    }

    getEpisodeMarkers(episodeId: number): Observable<EpisodeMarker[]> {
        return this.http.get<EpisodeMarker[]>(`${this.baseUrl}/episodes/${episodeId}/times`, { 
            headers: this.getAuthHeaders() 
        });
    }

    deleteEpisodeTrack(episodeId: number, type: 'audio' | 'subs', lang: string): Observable<any> {
        return this.http.delete(`${this.baseUrl}/episodes/${episodeId}/tracks/${type}/${lang}`, { 
            headers: this.getAuthHeaders() 
        });
    }

    uploadEpisodeMedia(file: File, type: 'thumbnail' | 'raw_video' | 'track'): Observable<{ uri: string }> {
        const formData = new FormData();
        let endpoint = '';
